...
class PaperDetector:
    """Detects and crops A4 paper from images using OpenCV"""
    
    def __init__(self, target_width: int = 2480, target_height: int = 3508):
        """
        Initialize detector with A4 dimensions at 300 DPI
    ...
    def detect_and_crop(self, image_path: str) -> Tuple[Optional[np.ndarray], dict]:
        """
        Detect paper in image and return cropped, perspective-corrected version
        
        Returns:
            cropped_image: Numpy array of the cropped paper
        ...
        if contour is None:
            return img, {"detected": False, "message": "No paper detected, using original"}
        ...
        # Resize to standard A4 dimensions
    ...
    def _find_paper_contour(self, img: np.ndarray) -> Optional[np.ndarray]:
        ...
        # Find the first contour with 4 corners (rectangle)
        for contour in contours:
            peri = cv2.arcLength(contour, True)
            approx = cv2.approxPolyDP(contour, 0.02 * peri, True)
            
            if len(approx) == 4:
                # Check if it's large enough (at least 20% of image)
...
def process_submission_image(image_path: str, output_path: str) -> dict:
    """
    Process a student submission image
    ...
    cropped_img, metadata = detector.detect_and_crop(image_path)
